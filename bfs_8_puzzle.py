...
def write_node_info(
    visited,
):  # To write all the info about the nodes explored by the program
    ...
    while node_q:
        ...
        for move in actions:
            ...
            if temp_data is not None:
                node_counter += 1
                child_node = Node(
                    node_counter, np.array(temp_data), current_root, move, 0
                )  # Create a child node

                if (
                    child_node.data.tolist() not in final_nodes
